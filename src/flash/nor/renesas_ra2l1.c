...
static int ra2l1_force_moco(struct target *t) {
    ...
    LOG_INFO(LOG_PREFIX ": force MOCO: before OSCSF=0x%02x, SCKSCR=0x%02x, ICKdiv=%u",
             oscsf, cksel, (unsigned)((div & SCKDIVCR_ICK_Msk) >> SCKDIVCR_ICK_Pos));
...
static int ra2l1_block_erase(struct flash_bank *bank, uint32_t addr) {
    ...
    LOG_INFO(LOG_PREFIX ": erase @0x%08x..0x%08x", start, end);
        ...
        if (info->is_data) {
            ...
            while (i < len) {
                ...
                if (w != 0xFFFFFFFFu) {
                    ...
                    if (rr) { ra2l1_exit_pe(t); return rr; }
                }
...
static void ra2l1_debug_dump_scan(const struct flash_bank *bank,
    ...
    while (b <= last_blk) {
        while (b <= last_blk && mark[b] == 0) b++;
        if (b > last_blk) break;
        uint32_t seg_first = b;
        while (b <= last_blk && mark[b] == 1) b++;
        ...
        if (seg_printed < MAX_SEG_PRINT) {
            ...
            LOG_INFO(LOG_PREFIX ": prescan seg[%u]: blocks=[%u..%u], off=[0x%08x..0x%08x], len=0x%08x",
...
static int ra2l1_flash_write(struct flash_bank *bank,
    ...
    LOG_INFO(LOG_PREFIX ": calc: blk_sz=%u, first_blk=%u, last_blk=%u, mark_slots=%u",
    ...
    while (b <= last_blk) {
        ...
        LOG_INFO(LOG_PREFIX ": SEG[%u] begin: blocks=[%u..%u], off=[0x%08x..0x%08x], len=0x%08x",
        ...
        LOG_INFO(LOG_PREFIX ": SEG[%u] done: elapsed=%lld ms",
    ...
    if (overall_ret == ERROR_OK) {
        LOG_INFO(LOG_PREFIX ": write exit: OK (segments=%u)", (unsigned)seg_idx);
    } else {
